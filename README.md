...
- **待办与提醒**：`Ctrl+T` 标记待办（**默认永久置顶、同样豁免淘汰**）；可设提醒时间（10 分钟后 / 30 分钟后 / 1 小时后 / 今天 18:00 / 明天 9:00），到点弹**右下角浮窗 + 系统提示音**，点击浮窗直达该条目；置顶可设时限，到期同样通知
- **右键菜单**：条目上右键即可完成上述全部操作（粘贴 / 收藏 / 待办 / 置顶时限 / 提醒 / 删除），不必记快捷键
- **置顶（可设时限）**：`Ctrl+P` 或右键把条目浮到"已固定"分组，支持永久或定时；保留天数照常生效
...
| 待办 / 取消待办 | `Ctrl+T`（默认永久置顶，豁免淘汰） |
| 置顶（含时限）/ 提醒 / 删除 | 条目上**右键**，菜单里选 |
| 置顶 / 取消置顶 | `Ctrl+P`（仅排序，仍受保留天数约束） |
| 删除 | `Ctrl+Delete` |

## 设置项

热键 · 触发词 · 条数上限（默认 5000）· 保留天数（默认 **90**，**0 = 不限时**，受条数上限兜底）· 暂停记录 · 显示来源进程 · 记录图片 · 单张图片上限（默认 20MB）· 重启保留历史（默认开，**文本明文落盘**，敏感内容请配合"暂停记录"）· 悬停预览 · **提醒提示音** · **待办按紧急度排序（临近时间越近越靠上，默认开）**
